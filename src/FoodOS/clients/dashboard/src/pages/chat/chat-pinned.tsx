    ...
    <DropdownMenu open={open} onOpenChange={setOpen}>
      <DropdownMenuTrigger asChild>
        <button
          ...
          aria-label={
            pinned.length === 1
              ? t("chat.pinnedAriaOne")
              : t("chat.pinnedAria").replace("{n}", String(pinned.length))
          }
          className={cn(
        ...
        >
          ...
          <span className="font-medium">
            {pinned.length === 1
              ...
              : t("chat.pinnedCount").replace("{n}", String(pinned.length))}
          </span>
          <ChevronDown
            className={cn(
    ...
    (message.attachments.length > 0
      ? `📎 ${message.attachments[0].originalFileName}${
          message.attachments.length > 1
            ? ` ${t("chat.moreAttachments").replace("{n}", String(message.attachments.length - 1))}`
            : ""
        }`
      : t("chat.noText"));
  return (
