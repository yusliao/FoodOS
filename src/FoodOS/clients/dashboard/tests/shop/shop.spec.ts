...
test.describe("shop/cart", () => {
  test("places an order from quoted cart lines", async ({ page }) => {
    await mockShopApis(page, {
      cartLines: [
        {
    ...
    let placed: unknown;
    await page.route("**/api/v1/ordering/orders", async (route) => {
    ...
    await mockJsonResponse(page, `**/api/v1/ordering/orders/${ORDER_ID}`, reservedOrder());
    ...
    await expect.poll(() => placed).toMatchObject({
      body: { storeId: STORE.id },
    });
    await expect(page).toHaveURL(new RegExp(`/shop/orders/${ORDER_ID}`));
  });
});

test.describe("shop/orders", () => {
  test("amends and cancels a reserved order before cutoff", async ({ page }) => {
    await mockShopApis(page);
    ...
    await page.route(`**/api/v1/ordering/orders/${ORDER_ID}/amend`, async (route) => {
      amendBody = JSON.parse(route.request().postData() ?? "{}");
      await route.fulfill({
        status: 200,
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify(ORDER_ID),
    ...
    let cancelled = false;
    await page.route(`**/api/v1/ordering/orders/${ORDER_ID}/cancel`, async (route) => {
      cancelled = true;
      await route.fulfill({
        status: 200,
        headers: { "Content-Type": "application/json" },
    ...
    await expect(page.getByText("You can amend or cancel until cutoff.")).toBeVisible();
    await expect(page.getByTestId("quoted-price").last()).toHaveText(/\$8\.00/);

    await page.getByLabel("Qty").fill("5");
    await page.getByRole("button", { name: /save changes/i }).click();
    await expect.poll(() => amendBody).toMatchObject({
      orderId: ORDER_ID,
