...
export function CategoriesPage() {
  ...
  return <div className="space-y-6">
    ...
    {editing !== undefined && ((editing && canUpdate) || (!editing && canCreate)) && <CategoryDialog category={editing} choices={query.data?.items ?? []} onClose={() => setEditing(undefined)} />}
    <ConfirmDialog open={!!deleting} onOpenChange={open => !open && setDeleting(null)} title={t("catalog.categories.deleteTitle")} description={t("catalog.deleteConfirm").replace("{name}", deleting?.name ?? "")} confirmLabel={t("catalog.delete")} destructive pending={remove.isPending} onConfirm={() => deleting && remove.mutate(deleting.id)} />
    {remove.isError && <ErrorBand message={describe(remove.error, t("catalog.requestFailed"))} />}
  </div>;
}

function CategoryDialog({ category, choices, onClose }: { category: CategoryDto | null; choices: CategoryDto[]; onClose: () => void }) {
  const t = useT();
  const cache = useQueryClient();
  const [name, setName] = useState(category?.name ?? "");
  const [description, setDescription] = useState(category?.description ?? "");
  const [parentCategoryId, setParentCategoryId] = useState(category?.parentCategoryId ?? "");
  const mutation = useMutation({ mutationFn: () => category ? updateCategory({ categoryId: category.id, name: name.trim(), description: description.trim(), parentCategoryId: parentCategoryId || null }) : createCategory({ name: name.trim(), description: description.trim(), parentCategoryId: parentCategoryId || null }), onSuccess: async () => { await cache.invalidateQueries({ queryKey: ["catalog", "categories"] }); toast.success(t(category ? "catalog.updated" : "catalog.created")); onClose(); } });
  function submit(event: FormEvent) { event.preventDefault(); if (name.trim() && !mutation.isPending) mutation.mutate(); }
  return <Dialog open onOpenChange={open => !open && !mutation.isPending && onClose()}><DialogContent><DialogHeader><DialogTitle>{t(category ? "catalog.categories.editTitle" : "catalog.categories.new")}</DialogTitle><DialogDescription>{t("catalog.categories.formHint")}</DialogDescription></DialogHeader><form onSubmit={submit}><DialogBody className="space-y-4"><fieldset disabled={mutation.isPending} className="space-y-4"><Field id="category-name" label={t("catalog.name")} required><Input id="category-name" required maxLength={128} value={name} onChange={event => setName(event.target.value)} /></Field><Field id="category-description" label={t("catalog.description")}><textarea id="category-description" className="min-h-24 w-full rounded-lg border border-[var(--color-border)] bg-transparent p-3" value={description} onChange={event => setDescription(event.target.value)} /></Field><Field id="category-parent" label={t("catalog.categories.parent")}><select id="category-parent" className="h-10 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] px-3" value={parentCategoryId} onChange={event => setParentCategoryId(event.target.value)}><option value="">{t("catalog.categories.noParent")}</option>{choices.filter(item => item.id !== category?.id).map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field></fieldset>{mutation.isError && <p role="alert" className="text-sm text-[var(--color-destructive)]">{describe(mutation.error, t("catalog.requestFailed"))}</p>}</DialogBody><DialogFooter><Button type="button" variant="outline" disabled={mutation.isPending} onClick={onClose}>{t("chrome.cancel")}</Button><Button type="submit" disabled={!name.trim() || mutation.isPending}>{t(mutation.isPending ? "catalog.saving" : "catalog.save")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
