...
import { changeProductPrice, createProduct, deleteProduct, searchBrands, searchCategories, searchProducts, updateProduct, type ProductDto } from "@/api/catalog";
...
export function ProductsPage() {
  ...
  const lookupsNeeded = editing !== undefined;
  const brands = useQuery({ queryKey: ["catalog", "brands", "product-form"], queryFn: ({ signal }) => searchBrands({ pageSize: 200 }, signal), enabled: lookupsNeeded && canReadLookups });
  const categories = useQuery({ queryKey: ["catalog", "categories", "product-form"], queryFn: ({ signal }) => searchCategories({ pageSize: 200 }, signal), enabled: lookupsNeeded && canReadLookups });
  ...
  return <div className="space-y-6">
    ...
    {editing !== undefined && ((editing && canEditProduct) || (!editing && canCreate)) && brands.isSuccess && categories.isSuccess && <ProductDialog product={editing} brands={brands.data.items} categories={categories.data.items} onClose={() => setEditing(undefined)} />}
    {editing !== undefined && (brands.isPending || categories.isPending) && <LoadingRow label={t("catalog.products.loadingForm")} />}
    {editing !== undefined && (brands.isError || categories.isError) && <ErrorBand message={t("catalog.products.lookupFailed")} />}
    {pricing && canUpdate && <PriceDialog product={pricing} onClose={() => setPricing(null)} />}
    <ConfirmDialog open={!!deleting} onOpenChange={open => !open && setDeleting(null)} title={t("catalog.products.deleteTitle")} description={t("catalog.deleteConfirm").replace("{name}", deleting?.name ?? "")} confirmLabel={t("catalog.delete")} destructive pending={remove.isPending} onConfirm={() => deleting && remove.mutate(deleting.id)} />
    {remove.isError && <ErrorBand message={describe(remove.error, t("catalog.requestFailed"))} />}
  </div>;
}

function ProductDialog({ product, brands, categories, onClose }: { product: ProductDto | null; brands: { id: string; name: string }[]; categories: { id: string; name: string }[]; onClose: () => void }) {
  const t = useT();
  const cache = useQueryClient();
  ...
  return <Dialog open onOpenChange={open => !open && !mutation.isPending && onClose()}><DialogContent size="lg"><DialogHeader><DialogTitle>{t(product ? "catalog.products.editTitle" : "catalog.products.new")}</DialogTitle><DialogDescription>{t("catalog.products.formHint")}</DialogDescription></DialogHeader><form onSubmit={submit}><DialogBody className="grid gap-4 sm:grid-cols-2"><fieldset disabled={mutation.isPending} className="contents"><Field id="product-sku" label={t("catalog.products.sku")} required><Input id="product-sku" required={!product} disabled={!!product} value={sku} onChange={event => setSku(event.target.value)} /></Field><Field id="product-name" label={t("catalog.name")} required><Input id="product-name" required value={name} onChange={event => setName(event.target.value)} /></Field><Field id="product-brand" label={t("catalog.products.brand")} required><select id="product-brand" className="h-10 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] px-3" required value={brandId} onChange={event => setBrandId(event.target.value)}><option value="">{t("catalog.products.chooseBrand")}</option>{brands.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field><Field id="product-category" label={t("catalog.products.category")} required><select id="product-category" className="h-10 w-full rounded-lg border border-[var(--color-border)] bg-[var(--color-card)] px-3" required value={categoryId} onChange={event => setCategoryId(event.target.value)}><option value="">{t("catalog.products.chooseCategory")}</option>{categories.map(item => <option key={item.id} value={item.id}>{item.name}</option>)}</select></Field>{!product && <Field id="product-price" label={t("catalog.products.priceUsd")} required><Input id="product-price" type="number" min="0" step="0.01" required value={amount} onChange={event => setAmount(event.target.value)} /></Field>}<label className="flex items-center gap-2 self-end pb-2 text-sm"><input type="checkbox" checked={isActive} onChange={event => setIsActive(event.target.checked)} />{t("catalog.products.active")}</label><div className="sm:col-span-2"><Field id="product-description" label={t("catalog.description")}><textarea id="product-description" className="min-h-24 w-full rounded-lg border border-[var(--color-border)] bg-transparent p-3" value={description} onChange={event => setDescription(event.target.value)} /></Field></div></fieldset>{mutation.isError && <p role="alert" className="sm:col-span-2 text-sm text-[var(--color-destructive)]">{describe(mutation.error, t("catalog.requestFailed"))}</p>}</DialogBody><DialogFooter><Button type="button" variant="outline" disabled={mutation.isPending} onClick={onClose}>{t("chrome.cancel")}</Button><Button type="submit" disabled={!valid || mutation.isPending}>{t(mutation.isPending ? "catalog.saving" : "catalog.save")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
...
function PriceDialog({ product, onClose }: { product: ProductDto; onClose: () => void }) {
  ...
  const cache = useQueryClient();
  const [amount, setAmount] = useState(String(product.price.amount));
  const mutation = useMutation({ mutationFn: () => changeProductPrice({ productId: product.id, amount: Number(amount), currency: product.price.currency }), onSuccess: async () => { await cache.invalidateQueries({ queryKey: ["catalog", "products"] }); toast.success(t("catalog.products.priceUpdated")); onClose(); } });
  function submit(event: FormEvent) { event.preventDefault(); if (Number(amount) >= 0 && !mutation.isPending) mutation.mutate(); }
  return <Dialog open onOpenChange={open => !open && !mutation.isPending && onClose()}><DialogContent size="sm"><DialogHeader><DialogTitle>{t("catalog.products.changePrice")}</DialogTitle><DialogDescription>{product.name} · {product.price.currency}</DialogDescription></DialogHeader><form onSubmit={submit}><DialogBody className="space-y-4"><Field id="price-amount" label={t("catalog.products.newPrice")} required><Input id="price-amount" type="number" min="0" step="0.01" required value={amount} onChange={event => setAmount(event.target.value)} /></Field>{mutation.isError && <p role="alert" className="text-sm text-[var(--color-destructive)]">{describe(mutation.error, t("catalog.requestFailed"))}</p>}</DialogBody><DialogFooter><Button type="button" variant="outline" disabled={mutation.isPending} onClick={onClose}>{t("chrome.cancel")}</Button><Button type="submit" disabled={Number(amount) < 0 || mutation.isPending}>{t(mutation.isPending ? "catalog.saving" : "catalog.save")}</Button></DialogFooter></form></DialogContent></Dialog>;
}
