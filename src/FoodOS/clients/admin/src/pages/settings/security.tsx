...
function makePasswordSchema(t: (key: string, fallback?: string) => string) {
  return z
    .object({
      current: z.string().min(1, t("settings.required")),
      next: z.string().min(8, t("auth.useAtLeast8")),
...
function ChangePasswordDialog({
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      ...
      toast.error(t("settings.changeFailed"), { description: detail });
    },
  });
    ...
    onError: (err: unknown) => {
      const detail =
        err instanceof ApiRequestError
          ? (err.problem?.detail ?? err.problem?.title ?? err.message)
          : (err as Error).message;
      toast.error(t("settings.enrollmentFailed"), { description: detail });
    },
  });
    ...
    onError: (err: unknown) => {
      const detail =
        err instanceof ApiRequestError
          ? (err.problem?.detail ?? err.problem?.title ?? err.message)
          : (err as Error).message;
      toast.error(t("settings.verifyFailed"), { description: detail });
    },
  });
    ...
    onError: (err: unknown) => {
      const detail =
        err instanceof ApiRequestError
          ? (err.problem?.detail ?? err.problem?.title ?? err.message)
          : (err as Error).message;
      toast.error(t("settings.disableFailed"), { description: detail });
    },
  });
