#!/usr/bin/env bash
#
# One-command deploy of the FoodOS Starter Kit to AWS.
#
...
if [[ "$BUILD_API" == true ]]; then
  ...
  dotnet publish "$REPO_ROOT/src/Host/FoodOS.Api/FoodOS.Api.csproj" \
  ...
  dotnet publish "$REPO_ROOT/src/Host/FoodOS.DbMigrator/FoodOS.DbMigrator.csproj" \
    -c Release -r linux-x64 \
    /t:PublishContainer \
    -p:ContainerRegistry="$REGISTRY_HOST" \
