    ...
    : ICommandHandler<CreateProductCommand, Guid>
{
    public async ValueTask<Guid> Handle(CreateProductCommand command, CancellationToken cancellationToken)
    {
        ...
        var product = Product.Create(
            ...
            command.Stock,
            ParseZone(command.TemperatureZone),
    ...
    private static TemperatureZone ParseZone(string? value)
        => Enum.TryParse(value, ignoreCase: true, out TemperatureZone zone)
            ? zone
            : throw new CustomException(
                $"Unknown temperature zone '{value}'.",
                (IEnumerable<string>?)null,
