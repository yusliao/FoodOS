...
dotnet run --project src/Host/FoodOS.DbMigrator -- apply

# Apply only to one tenant.
dotnet run --project src/Host/FoodOS.DbMigrator -- apply --tenant root

# Apply only the tenant catalog (no per-tenant pass).
dotnet run --project src/Host/FoodOS.DbMigrator -- apply --catalog-only
...
as `FoodOS.Api` (both files are linked into the project so they
...
| `DatabaseOptions__MigrationsAssembly`     | `FoodOS.Migrations.PostgreSQL`         |
...
```bash
dotnet publish src/Host/FoodOS.DbMigrator -c Release \
        ...
        - name: migrator
          ...
          env:
            ...
            - name: DatabaseOptions__MigrationsAssembly
              value: FoodOS.Migrations.PostgreSQL
```
...
- **Aspire**: `dotnet run --project src/Host/FoodOS.AppHost` —
...
- **Raw**: run the migrator once after pulling, before starting the
  ...
  # Then start the API
  dotnet run --project src/Host/FoodOS.Api
  ```
