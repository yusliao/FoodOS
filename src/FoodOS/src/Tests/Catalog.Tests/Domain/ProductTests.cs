...
public sealed class ProductTests
{
    ...
    #region Fulfillment + translations

    [Fact]
    public void SetFulfillmentAttributes_Should_NormalizeUomAndPersistZone()
    {
        Product product = CreateValidProduct();
        ...
        product.SetFulfillmentAttributes(
            ...
            shelfLifeDays: 90,
            minRemainingDaysOnShip: 7,
            baseUom: " lb ",
            catchWeight: true,
            barcode: " 012345 ",
            storageNote: " keep frozen ");

        product.TemperatureZone.ShouldBe(TemperatureZone.Frozen);
        product.ShelfLifeDays.ShouldBe(90);
        product.MinRemainingDaysOnShip.ShouldBe(7);
        product.BaseUom.ShouldBe("LB");
        product.CatchWeight.ShouldBeTrue();
        product.Barcode.ShouldBe("012345");
        product.StorageNote.ShouldBe("keep frozen");
    }

    [Fact]
    public void UpsertTranslation_Should_FallbackToCanonicalName_When_CultureMissing()
    {
        Product product = CreateValidProduct();
        product.UpsertTranslation("es-ES", "Producto de prueba", "descripción");

        (string Name, string? Description) spanish = product.ResolveLocalizedCopy("es-ES");
        spanish.Name.ShouldBe("Producto de prueba");
        spanish.Description.ShouldBe("descripción");

        (string Name, string? Description) german = product.ResolveLocalizedCopy("de-DE");
        german.Name.ShouldBe(product.Name);
        german.Description.ShouldBe(product.Description);
    }

    [Fact]
    public void UpsertTranslation_Should_MatchLanguageWhenRegionDiffers()
    {
        Product product = CreateValidProduct();
        product.UpsertTranslation("fr-FR", "Produit d'essai", null);
