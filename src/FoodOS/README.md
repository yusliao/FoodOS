...
```bash
dotnet run --project src/Host/FoodOS.AppHost
```
...
```bash
dotnet run --project src/Host/FoodOS.Api      # needs external Postgres + Redis
```
...
src/
  ...
  Host/
    FoodOS.Api/                    API composition root
    FoodOS.AppHost/                .NET Aspire orchestrator
    FoodOS.DbMigrator/             One-shot migrate / seed runner
    FoodOS.Migrations.PostgreSQL/  EF Core migrations
...
- [ ] **Mail** — configure SMTP / SendGrid under `MailOptions` in
      `src/Host/FoodOS.Api/appsettings.json`.
...
```bash
dotnet test src/FoodOS.slnx       # integration tests require Docker
```
