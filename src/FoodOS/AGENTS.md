...
| `src/Host/FoodOS.Api` | Composition-root Web API host. |
| `src/Host/FoodOS.AppHost` | .NET Aspire orchestrator (Postgres, Redis, MinIO, migrator, API, **both React apps**). |
| `src/Host/FoodOS.DbMigrator` | One-shot migrate/seed runner. DB is **not** migrated at API startup. |
| `src/Host/FoodOS.Migrations.PostgreSQL` | All EF migrations, organized per-module by folder. |
...
dotnet run --project src/Host/FoodOS.AppHost   # one-time: npm install in clients/admin & clients/dashboard

dotnet build src/FoodOS.slnx                   # build backend
dotnet run --project src/Host/FoodOS.Api       # API only → https://localhost:7030 (/scalar)
dotnet test src/FoodOS.slnx                    # tests — integration tests REQUIRE Docker
...
```bash
dotnet run --project src/Host/FoodOS.DbMigrator -- apply [--seed]
dotnet run --project src/Host/FoodOS.DbMigrator -- list-pending
```
