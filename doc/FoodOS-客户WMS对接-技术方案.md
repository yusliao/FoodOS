...
> 文档用途：双方技术评审的方案基线。
>
> 状态：2026-09-24 FoodOS 已按用户决定制定 `FoodOS WMS Standard v1` 并开始实现，不再等待仓库方先给协议。仓库方适配依据为 [OpenAPI 契约](contracts/foodos-wms-v1.openapi.yaml)。业务责任仍以 [已确认决策](FoodOS-客户WMS对接-核心问题决策建议.md) 为准；开发环境验证不代表生产切换授权。

### 1.0 标准版本与兼容规则

- 当前协议版本为 `1.0.0`，事件信封 `schemaVersion` 固定为 `1.0`。
- 仓库方实现健康、预占、释放、结果查询、采购入库任务、销售出库任务及取消接口；FoodOS 实现带签名的事件接收接口。
...
WMS 是正式预占和分配权威；FoodOS 不重复扣减。WMS 不可用时只允许浏览、购物车或待确认需求，不承诺库存和配送。超时结果未知必须查询确认，不能当作明确失败后换键重下。上述接口与状态衔接尚待实现和联合验证。

### 7.4 取消和改单

需要区分：

- 尚未下发：按 FoodOS 截单规则撤销；若已有 WMS 预占，仍须取得释放确认。
- 已下发但接单结果未知：先查最终结果，不将超时当成未接单。
- WMS 已接单未拣货：请求取消，等待明确确认。
- WMS 已拣货：不能静默取消，应进入异常/反向作业流程。
- WMS 已发运：不能按取消处理，应走退货或拒收。

交仓后改单和取消须 WMS 明确确认。正式预占由 WMS 释放并回传结果，FoodOS 再更新订单与可售视图；不提前显示取消成功或自行释放另一套库存。
...
在这些问题没有验证前，不得直接将 FoodOS ATP 替换成 WMS 的一个库存数值字段并开放正式接单。期初快照必须接续增量，未完成采购、待拣、已拣未发、已发未签与退货单须明确衔接。
