...
FoodOS 仅在以下条件全部满足时向饭店端开放“提交订单”：连接配置完整；配置的试点仓库和运营货主映射处于启用状态；仓库适配器 `GET /api/v1/health` 返回 2xx。健康接口不可达、超时或返回非成功状态时，FoodOS 继续允许浏览和维护购物车，但关闭提交入口。SKU 与单位映射按每笔订单逐行校验，健康状态不替代业务映射校验。
